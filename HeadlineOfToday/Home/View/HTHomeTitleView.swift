...
protocol HTHomeTitleViewDelegate: class {
    func titleView(_ titleView: HTHomeTitleView, targetIndex: Int)
}

class HTHomeTitleView: UIView {

    weak var delegate: HTHomeTitleViewDelegate?
    
    var titles: [HTHomeTopicTitle]? {
        didSet {
            // 将titleLabel添加到ScrollView中
            setupTitleLabels()
            // 设置每个titleLabel的frame
    ...
    /// 标题数组
    fileprivate lazy var titleLabels = [HTHomeTitleLabel]()
    /// 滚动试图
    ...
    fileprivate lazy var rightButton: UIButton = {
        ...
        rightButton.setImage(#imageLiteral(resourceName: "add_channel_titlbar_thin_new_16x16_"), for: .normal)
        rightButton.setBackgroundImage(#imageLiteral(resourceName: "shadow_add_titlebar_new3_52x36_"), for: .normal)
...
extension HTHomeTitleView {
    ...
    /// 添加titleLabel到scrollView中
    fileprivate func setupTitleLabels() {
        for (index, topTitle) in titles!.enumerated() {
            let titleLabel = HTHomeTitleLabel()
    ...
    fileprivate func setupTitleLabelsFrame() {
        ...
        for (i, label) in titleLabels.enumerated() {
            ...
            w = topTitle.name!.boundingRect(with: CGSize(width: CGFloat(MAXFLOAT), height: 0), options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font: label.font], context: nil).width
            if i == 0 {
                x = kMargin * 0.5
                bottomLine.x = x
                bottomLine.width = w
...
extension HTHomeTitleView {
    ...
    @objc fileprivate func titleLabelClick(_ tapGes: UITapGestureRecognizer) {
        // 取出用户点击的View
        let targetLabel = tapGes.view as! HTHomeTitleLabel
        // 调整title
...
extension HTHomeTitleView: HTHomePageViewDelegate {
    func pageView(_ pageView: HTHomePageView, targetIndex: Int) {
        adjustTitleLabel(targetIndex: targetIndex)
    }
}

private class HTHomeTitleLabel: UILabel {
    // 用来记录当前label的缩放比例
