...
protocol NetworkToolProtocol {
    ...
    /// -------------------------- 首 页 home -------------------------
    /// 获取首页顶部标题内容
    static func loadHomeTitlesData(fromViewController: String, completionHandler:@escaping (_ topTitles: [HTHomeTopicTitle], _ homeTopicVCs: [HTHomeTopicViewController])->())
    /// 获取首页顶部标题内容
    static func loadHomeTitlesData(completionHandler:@escaping (_ topTitles: [HTHomeTopicTitle])->())
    /// 点击首页加号按钮，获取频道推荐数据
    static func loadHomeCategoryRecommend(completionHandler:@escaping (_ topTitles: [HTHomeTopicTitle]) -> ())
    /// 搜索
    ...
    /// 获取头条号 关注
    static func loadEntryList(completionHandler:@escaping (_ concerns: [HTHomeConcernToutiaohao])->())
    /// 悟空问答
    static func loadQuestionAnswerList(topicTitle: HTHomeTopicTitle, weitoutiao: WeiTouTiao, completionHandler:@escaping (_ questionAnswer: QuestionAnswer)->())
    /// -------------------------- 视 频 video --------------------------
    /// 获取视频顶部标题内容
    static func loadVideoTitlesData(completionHandler:@escaping (_ videoTitles: [HTHomeTopicTitle], _ videoTopicVCs: [HTVideoTopicController])->())
    // --------------------------  微  头  条  --------------------------
    /// 获取微头条数据
...
class NetworkTool: NetworkToolProtocol {
    ...
    // MARK: - 获取首页顶部标题内容
    /// 获取首页顶部标题内容
    class func loadHomeTitlesData(fromViewController: String, completionHandler:@escaping (_ topTitles: [HTHomeTopicTitle], _ homeTopicVCs: [HTHomeTopicViewController])->()) {
        ...
        Alamofire.request(url, parameters: params).responseJSON { (response) in
            ...
            if let value = response.result.value {
                ...
                if let data = dataDict!["data"]!.arrayObject {
                    ...
                    var homeTopicVCs = [HTHomeTopicViewController]()
                    // 添加推荐标题
                    let recommendDict = ["category": "", "name": "推荐"]
                    let recommend = HTHomeTopicTitle(dict: recommendDict as [String : AnyObject])
                    titles.append(recommend)
                    // 添加控制器
                    let firstVC = HTHomeTopicViewController()
                    firstVC.topicTitle = recommend
                    homeTopicVCs.append(firstVC)
                    for dict in data {
                        let topicTitle = HTHomeTopicTitle(dict: dict as! [String: AnyObject])
                        titles.append(topicTitle)
                        let homeTopicVC = HTHomeTopicViewController()
    ...
    /// 获取首页顶部标题内容
    class func loadHomeTitlesData(completionHandler:@escaping (_ topTitles: [HTHomeTopicTitle])->()) {
        let url = BASE_URL + "article/category/get_subscribed/v1/?"
        let params = ["device_id": device_id,
                      "aid": 13,
                      "iid": IID] as [String : AnyObject]
    ...
    /// 点击首页加号按钮，获取频道推荐数据
    class func loadHomeCategoryRecommend(completionHandler:@escaping (_ topTitles: [HTHomeTopicTitle]) -> ()) {
    ...
    /// 获取头条号 关注
    class func loadEntryList(completionHandler:@escaping (_ concerns: [HTHomeConcernToutiaohao])->()) {
        ...
        Alamofire.request(url, parameters: params).responseJSON { (response) in
            ...
            if let value = response.result.value {
                let json = JSON(value)
                if let data = json["data"].arrayObject {
                    var concerns = [HTHomeConcernToutiaohao]()
                    for item in data {
                        let concern = HTHomeConcernToutiaohao(dict: item as! [String : AnyObject])
    ...
    /// 悟空问答
    class func loadQuestionAnswerList(topicTitle: HTHomeTopicTitle, weitoutiao: WeiTouTiao, completionHandler:@escaping (_ questionAnswer: QuestionAnswer)->()) {
    ...
    /// 获取视频顶部标题内容
    class func loadVideoTitlesData(completionHandler:@escaping (_ videoTitles: [HTHomeTopicTitle], _ videoTopicVCs: [HTVideoTopicController])->()) {
        ...
        Alamofire.request(url, parameters: params).responseJSON { (response) in
            ...
            if let value = response.result.value {
                ...
                if let data = json["data"].arrayObject {
                    var titles = [HTHomeTopicTitle]()
                    var videoTopicVCs = [HTVideoTopicController]()
                    // 添加推荐标题
                    let recommendDict = ["category": "video", "name": "推荐"]
                    let recommend = HTHomeTopicTitle(dict: recommendDict as [String : AnyObject])
                    titles.append(recommend)
                    // 添加控制器
                    let firstVC = HTVideoTopicController()
                    firstVC.videoTitle = recommend
                    videoTopicVCs.append(firstVC)
                    for dict in data {
                        let title = HTHomeTopicTitle(dict: dict as! [String: AnyObject])
                        let videoTopicVC = HTVideoTopicController()
                ...
                if let data = json["data"].dictionary {
                    if let sections = data["sections"]?.arrayObject {
                        ...
                        for section in sections {
                            var rows = [HTMineCellModel]()
                            for row in section as! [AnyObject] {
                                let mineCell = HTMineCellModel(dict: row as! [String : AnyObject])
