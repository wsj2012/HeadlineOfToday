...
class HTVideoTopicController: UIViewController {
    ...
    /// 上一次点击的 cell
    var lastCell = HTVideoTopicCell()
    
    // 记录点击的顶部标题
    var videoTitle: HTHomeTopicTitle?
    // 存放新闻主题的数组
    ...
    fileprivate lazy var tableView: UITableView = {
        ...
        tableView.ym_registerCell(cell: HTVideoTopicCell.self)
        tableView.backgroundColor = UIColor.init(r: 246, g: 246, b: 246)
        return tableView
    }()
}

extension HTVideoTopicController {
...
extension HTVideoTopicController: UITableViewDelegate, UITableViewDataSource {
    // MARK: - Table view data source
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.ym_dequeueReusableCell(indexPath: indexPath) as HTVideoTopicCell
        ...
        cell.bgImageButton.rx.controlEvent(.touchUpInside)
            .subscribe(onNext: { (_) in
                ...
                self.player = BMPlayer(customControlView: HTBMPlayerCustomControlView())
