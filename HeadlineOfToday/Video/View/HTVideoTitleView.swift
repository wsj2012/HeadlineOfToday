...
protocol HTVideoTitleViewDelegate: class {
    
    func videoTitle(videoTitle: HTVideoTitleView, didClickSearchButton searchButton: UIButton)
    /// 点击了标题
    func titleView(_ titleView: HTVideoTitleView, targetIndex : Int)
}

class HTVideoTitleView: UIView {

    weak var delegate: HTVideoTitleViewDelegate?
    ...
    fileprivate let style = HTHomeTitleStyle()
    ...
    fileprivate lazy var titleSearchButton: UIButton = {
        ...
        titleSearchButton.setImage(#imageLiteral(resourceName: "search_24x24_"), for: .normal)
...
extension HTVideoTitleView {
    ...
    fileprivate func setupTitleLabels(videoTitles: [HTHomeTopicTitle]) {
        for (index, topTitle) in videoTitles.enumerated() {
            ...
            titleLabel.textColor = index == 0 ? UIColor.init(r: 210, g: 63, b: 66) : UIColor.black
    ...
    /// 调整 label 的 frame
    fileprivate func setupTitleLabelsFrame(titleLabels: [TitleLabel], titles: [HTHomeTopicTitle]) {
...
extension HTVideoTitleView: HTVideoViewControllerDelegate {
